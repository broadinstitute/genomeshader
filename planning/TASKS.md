...
- [ ] **#78** MEASURE: does core-aware parallel window decode speed first-visit (esp. remote)? First-visit to a NEW window is read-bound (htslib reading ~500KB/record text-VCF lines at 20k samples). Split window → N readers on detected cores (2-4) → merge. MEASURE-FIRST: local vs remote (N reader-opens = N TLS + N index fetches may eat the gain). Extend scripts/bench_pf7.py; go/no-go from data. Flagged important by Jonn
