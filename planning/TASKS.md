...
- [ ] **#81** Vertical smart track: alpha-blended read bodies don't paint on the FIRST frame — appear only after a canvas-context reconfigure (a resize). Pre-existing WebGPU first-frame quirk (not from #80; a real interactive session re-renders constantly). LOW PRIORITY
- [ ] **#82** Vertical mode cosmetic polish: axis tick-label overlap with the ideogram/reference gutter ("100…0 kb" garble) + leftmost track-header edge clip behind the sidebar. LOW PRIORITY
...
- [x] **#80** Vertical mode: sample (smart/read) track renders as a vertical PILEUP. Root cause was a W/H axis swap in the vertical render branch (genomic axis = W = container height; read columns across cross axis = H = container width) — reads crammed into the top ~24%, grid lines only top quarter, variant guides drawn offscreen. Fixed to map genomic dims over W and cross-axis dims over H. GPU-verified on the Quadro RTX 8000: read-body instances now span ~80% of the axis. Guard `test_vertical_reads_span_genomic_axis` (geometry-based, immune to #81). Remainders split → #81 (first-frame paint quirk) + #82 (axis-label/header cosmetics)
- [x] **#78** Parallel window decode — MEASURED, verdict DEFER. `scripts/bench_parallel_decode.py` (bit-exact split, asserted). Decode is CPU-parallelizable but: N>2 never pays (matches "assume two"); the common case — remote pan to a fresh tight window — gets only 1.32x and N=4==N=2 (coordinate-split re-fetches shared 64KB bgzf boundary blocks). #77 cache already frees revisits. Higher-leverage levers: #71 overscan prefetch (shipped) + P3 density LOD (deferred). If ever built: N=2 only, byte/bgzf-offset split, wide cold loads only. Full matrix + mechanism in JOURNAL 2026-09-03. Pf7 tabix `.tbi` is KB regardless of chr (chr1 .tbi = 2268 B), so the "N .tbi fetches" fear was unfounded
- [x] **#77** Large-scale fast on modest machines (CORE). Remote window-local seek verified on real 36GB chr1; bounded host LRU cache (revisit 19,000ms→0.1ms on real Pf7); aggregate-default at ≥5k (was ≥100k, so 50k malaria now uses the fast path); allocation-free bit-exact decode. Profiling truth: the wall is htslib reading the raw text-VCF lines (~500KB/record at 20k samples) — inherent to text VCF; cache amortizes it. First-visit lever → #78. Commits 6f0ca5a/ea4f76f/0f1864d
