...
impl IndexedReader {
    ...
    fn new(path: &ffi::CStr) -> Result<Self> {
        ...
        unsafe {
          ...
          //
          // htslib documents bcf_sr_add_reader as returning 1 on success and 0
          // on error. Upstream rust-htslib 0.44.1 treated `>= 0` as success, so a
          // failed remote (gs://) open was followed by a NULL-header deref and
          // SIGSEGV — which kills a Jupyter kernel. SyncedReader::add_reader in
          // this same crate already checks `res == 0`.
        ...
        if added > 0 {
            ...
            let header = Rc::new(HeaderView::new(unsafe { htslib::bcf_hdr_dup(hdr_ptr) }));
            Ok(IndexedReader {
                inner: ser_reader,
                header,
                current_region: None,
